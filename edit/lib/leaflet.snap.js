...
L.Handler.MarkerSnap = L.Handler.extend({
    ...
    enable: function () {
        ...
        function computeBuffer() {
            this._buffer = window.api.map.layerPointToLatLng(new L.Point(0,0)).lat -
                           window.api.map.layerPointToLatLng(new L.Point(this.options.snapDistance, 0)).lat;
        }
        window.api.map.on('zoomend', computeBuffer, this);
        window.api.map.whenReady(computeBuffer, this);
    ...
    _snapMarker: function(e) {
        ...
        var closest = this._findClosestLayerSnap(snaplist,
                                                 latlng,
    ...
    _findClosestLayerSnap: function (layers, latlng, tolerance, withVertices) {
        return L.GeometryUtil.closestLayerSnap(window.api.map, layers, latlng, tolerance, withVertices);
    },
...
L.Draw.Polyline.prototype._onMouseUp = function (e) {
  if (this._mouseDownOrigin) {
    ...
    if (Math.abs(distance) < 9 * (window.devicePixelRatio || 1)) {
      this.addVertex(this._mouseMarker.getLatLng());
    }
  }
...
L.Handler.PolylineSnap = L.Edit.Poly.extend({
    ...
    initialize: function (poly, options) {
        ...
        this._snapper = new L.Handler.MarkerSnap(options);
    },
